...
def test_main():
    ...
    assert isinstance(app.main, types.FunctionType), (
        'Проверьте, что `main` - это функция.'
    )


def test_read_package():
    assert hasattr(app, 'read_package'), (
        'Создайте функцию для обработки '
        'входящего пакета - `read_package`'
    )
    assert callable(app.read_package), (
        'Проверьте, что `read_package` - это функция.'
    )
    assert isinstance(app.read_package, types.FunctionType), (
        'Проверьте, что `read_package` - это функция.'
    )


def test_InfoMessage():
    assert inspect.isclass(app.InfoMessage), (
...
def test_Training():
    assert inspect.isclass(app.Training), (
...
def test_Swimming():
    ...
    assert inspect.isclass(app.Swimming), (
        'Проверьте, что `Swimming` - это класс.'
    )
    assert issubclass(app.Swimming, app.Training), (
...
def test_Walking():
    assert hasattr(app, 'Walking'), 'Создайте класс `Walking`'
    assert inspect.isclass(app.Walking), (
        'Проверьте, что  `Walking` - это класс.'
    )
    assert issubclass(app.Walking, app.Training), (
        'Класс `Walking` должен наследоваться от класса `Training`.'
    )
    sports_walking = app.Walking
    ...
    for param in ['action', 'duration', 'weight', 'height']:
        assert param in sports_walking_signature_list, (
            'У метода `__init__` класса `Walking` '
            f'должен быть параметр {param}.'
        )
...
def test_Walking_get_spent_calories(input_data, expected):
    sports_walking = app.Walking(*input_data)
    result = sports_walking.get_spent_calories()
    assert type(result) == float, (
        'Переопределите метод `get_spent_calories` в классе `Walking`.'
    )
    assert result == expected, (
        'Проверьте формулу подсчёта потраченных '
        'калорий в классе `Walking`'
    )


def test_Running():
    assert hasattr(app, 'Running'), 'Создайте класс `Running`'
    assert inspect.isclass(app.Running), (
        'Проверьте, что `Running` - это класс.'
    )
    assert issubclass(app.Running, app.Training), (
